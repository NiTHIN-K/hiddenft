...
//   async function mint(){
  ...
  function Attachments(){
    ...
    for(let i = 0; i < theFiles.length; i++){
      elements.push(<p>{theFiles[i].name}</p>)
    }
    return (
      <div>
        {elements}
  ...
  function dropFiles(event) {
    //event.dataTransfer.files.forEach((file, i) => {theFiles.push(file)})
    console.log(event.dataTransfer.items.length)
      let newFiles = [...theFiles]
    for (let i = 0; i < event.dataTransfer.items.length; i++) {
      newFiles.push(event.dataTransfer.items[i].getAsFile())
    }
      setTheFiles(newFiles)
    // for (let key in event.dataTransfer.files){
    //   console.log(key)
    //   theFiles.push(event.dataTransfer.files[key])
    // }
